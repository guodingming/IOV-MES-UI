<template>
	<el-dialog title="新建工作中心" :visible.sync="show" size="full" :close-on-click-modal="false">
...
<script>
	...
	import createForm from "./workcenterCreate"
	import editForm from "./workcenterEdit"
...
//          },
		...
		methods: {
			...
			async queryData() {
				...
				let data = {
					condition: {
            productionLineId: this.bringData.id,
						search:this.searchData
					},
					pageSize: this.table.pageSize,
					pageNum: this.table.pageNum
				}
				let res = await workcenterQueryData(data);
			...
			delete_getPage(data) {
				this.$.delete('此操作将永久删除该工作中心, 是否继续?',() => {
					workcenterDeleting({
						ids: data.id
					}).then((res) => {
						this.queryData()
			...
			multiDelete() {
				this.$.delete('此操作将永久删除所选工作中心, 是否继续?',() => {
					workcenterDeleting({
