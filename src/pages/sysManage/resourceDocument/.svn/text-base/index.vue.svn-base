	...
	<div id="resourceDocument">
		<mes-table-layout :tree-data="treeData" @tree-click="treeClick" v-if="treeData">
			...
			<div class="mes-content" slot="container">
				<div class="mes-tool">
					<el-button type="primary" @click="create_getPage":disabled="!selectNode||(selectNode&&selectNode.isRoot)">导入<i class="el-icon-upload el-icon--right"></i>
					</el-button>
                ...
                <el-button @click.native.prevent="configBOM_getPage(scope.row)" type="text" size="small">
                  配置物料
                </el-button>
		...
		<configBOMForm ref="configBOM_form"></configBOMForm>
...
<script>
	...
	import configBOMForm from "./configBOM"
	...
	export default {
      ...
      editForm
		...
		methods: {
			...
			async getTree() {
				let res = await getTree({type:this.fileType});
        ...
        this.mutiDeleteData = [];
				let data = {
					condition: {
						fileTypeId: this.selectNode ? this.selectNode.id : "",
			...
			configBOM_getPage(data) {
				this.$refs.configBOM_form.getPage(data)
			},
			treeCreate_getPage() {
			...
			treeDelete_getPage() {
				this.$.delete('此操作将永久删除该文件分类, 是否继续?',() => {
					treeDelete({
