<template>
	<div id="siteManage">
			...
			<div class="mes-content" slot="container">
				...
				<div class="mes-table" v-if="table.data">
					<mes-table :table-data="table.data"
						...
						<el-table-column fixed="right" label="操作" width="170" v-if="table.data.length>0" slot="operationTool">
							<template scope="scope">
								<el-button @click.native.prevent="edit_getPage(scope.row)" type="text" size="small">
									编辑
								</el-button>
								<el-button @click.native.prevent="area_getPage(scope.row)" type="text" size="small">
									车间管理
								</el-button>
		...
		<areaForm ref="area_form"></areaForm>
	</div>
</template>
...
<script>
	...
	export default {
        ...
        this.mutiDeleteData = [];
				let data = {
					condition: {
						enterpriseId: this.selectNode ? this.selectNode.id : "",
			...
			area_getPage(data){
				this.$refs.area_form.getPage(data)
			},
			delete_getPage(data) {
				this.$.delete('此操作将永久删除该工厂, 是否继续?',() => {
					deleting({
						ids: data.id
					}).then((res) => {
						this.queryData()
