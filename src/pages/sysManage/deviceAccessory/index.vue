...
<script>
  ...
  async queryData() {
    ...
    let data = {
      condition: {
        deviceAnnexId: this.selectNode ? this.selectNode.id : "",
  ...
  delete_getPage(data) {
    this.$.delete('此操作将永久删除该附件信息, 是否继续?',() => {
      deleting({
        ids: data.id
  }).then((res) => {
      this.queryData()
  ...
  multiDelete() {
    this.$.delete('此操作将永久删除所选附件信息, 是否继续?',() => {
      deleting({
