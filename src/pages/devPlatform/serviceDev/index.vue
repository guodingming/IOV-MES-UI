          ...
          <mes-table :table-data="table.data"
            ...
            <el-table-column fixed="right" label="操作" width="160" v-if="table.data.length>0" slot="operationTool">
              <template scope="scope">
                <el-button @click.native.prevent="edit_getPage(scope.row)" type="text" size="small" :disabled="!selectNode||(selectNode&&selectNode.isRoot)">
                  编辑
                </el-button>
                <el-button @click.native.prevent="test_getPage(scope.row)" type="text" size="small">
                  测试
                </el-button>
...
<script>
  ...
  async queryData() {
    ...
    let data = {
      condition: {
        serviceTypeId: this.selectNode ? this.selectNode.id : "",
  ...
  treeDelete_getPage() {
    this.$.delete('此操作将永久删除该接口分类, 是否继续?',() => {
      treeDelete({
