      ...
      <ul>
        <li v-for="value in data" :key="value.id">
          <router-link :to="value.url">
            <div class="menuIcon"><img :src="require('assets/images/index/' + value.icon)"/></div>
...
<style>
  ...
  .mes__header .menu ul li a .menuIcon {
    height: 42px;
    width: 96px;
    margin: 10px auto;
    overflow: hidden;
  ...
  .mes__header .menu ul li a .menuIcon img {
    position: relative;
  }
  ...
  .mes__header .menu ul li a.router-link-active img {
    top: -42px
  }
