<template>
	<div ref="mesTable" class="mes__table">
		...
		<el-table v-if="ready"
			...
			<el-table-column fixed v-if="showCheckBox" type="selection" width="40"></el-table-column>
			<el-table-column fixed type="index" label="序号" width="70"></el-table-column>
			<el-table-column v-for="item in tableOption" sortable :key="item.id" :prop="item.prop" :label="item.label" :min-width="item.width"></el-table-column>
			<slot name="prop"></slot>
...
<script>
	export default {
		props:['tableOption','tableData','tableLoading','total','pageSize','pageChange','sizeChange','showCheckBox','maxHeight'],
		data() {
			return {
						...
						callbacks:{
						    whileScrolling:() =>{
								header = $(this.$refs.mesTable).find(".el-table").children(".el-table__header-wrapper")
						     	...
						     	$(this.$refs.mesTable).find(".el-table__fixed-body-wrapper").css({"max-height":scollDom.css("max-height")})
								let left = -parseInt($(this.$refs.mesTable).find(".mCSB_container").css("left"))
						     	...
						     	if(header.length>0){header.scrollLeft(left)}
						     	if(footer.length>0){footer[0].scrollLeft = left}
						     	if(fixedBody.length>0){fixedBody.find(".el-table__fixed-body-wrapper")[0].scrollTop = top}
						     	if(fixedRight.length>0){fixedRight.find(".el-table__fixed-body-wrapper")[0].scrollTop = top}
						    }
						}
					});
					})
